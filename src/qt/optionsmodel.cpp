...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::VCO);
    ...
    if (!settings.contains("nAnonymizeVoucherCoinAmount"))
        settings.setValue("nAnonymizeVoucherCoinAmount", 1000);

    nAnonymizeVoucherCoinAmount = settings.value("nAnonymizeVoucherCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVoucherCoinAmount"))
        SoftSetArg("-anonymizevouchercoinamount", settings.value("nAnonymizeVoucherCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in vouchercoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVoucherCoinAmount:
            return QVariant(nAnonymizeVoucherCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVoucherCoinAmount:
            nAnonymizeVoucherCoinAmount = value.toInt();
            settings.setValue("nAnonymizeVoucherCoinAmount", nAnonymizeVoucherCoinAmount);
            emit anonymizeVoucherCoinAmountChanged(nAnonymizeVoucherCoinAmount);
            break;
        case CoinControlFeatures:
