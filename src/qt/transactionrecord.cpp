...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zvcoTracker->HasMintTx(hash))) {
            //zVCO stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZVCO;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by VoucherCoin Address
...
bool IsZVCOType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZVCO:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZVCO || type == TransactionRecord::MNReward) {
