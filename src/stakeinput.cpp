...
// The zVCO block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zVCO is a hash of the serial
...
{
    //Create an output returning the zVCO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZVCOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zVCO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zVCO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZVCOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zVCO output", __func__);
...
{
    CzVCOTracker* zvcoTracker = pwallet->zvcoTracker.get();
    CMintMeta meta;
    if (!zvcoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zvcoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a VCO stake is the outpoint
