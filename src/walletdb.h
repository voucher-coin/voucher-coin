...
public:
    ...
    bool WriteZVCOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVCOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVCOSeed_deprecated(uint256& seed);
    bool EraseZVCOSeed();
    bool EraseZVCOSeed_deprecated();

    bool WriteZVCOCount(const uint32_t& nCount);
    bool ReadZVCOCount(uint32_t& nCount);
