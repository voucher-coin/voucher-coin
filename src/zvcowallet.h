...
#ifndef VoucherCoin_ZVCOWALLET_H
#define VoucherCoin_ZVCOWALLET_H
...
public:
    CzVCOWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZVCO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZVCO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //VoucherCoin_ZVCOWALLET_H
