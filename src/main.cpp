...
/** Fees smaller than this (in uvco) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zVCO is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zVCO spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zVCO without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zVCO spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zVCO spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zVCO spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvco");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zVCO mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight <= 30001 && nHeight >= 20001) {
        nSubsidy = 30 * COIN;
    }
    else if (nHeight <= 530002 && nHeight >= 30002) {
        nSubsidy = 2.5 * COIN;
    }
    else if (nHeight <= 540003 && nHeight >= 530003) {
        nSubsidy = 30 * COIN;
    }
    else if (nHeight <= 1040004 && nHeight >= 540004) {
        nSubsidy = 3 * COIN;
    }
    else if (nHeight <= 1050005 && nHeight >= 1040005) {
        nSubsidy = 30 * COIN;
    }
    else if (nHeight <= 1550006 && nHeight >= 1050006) {
        nSubsidy = 2.5 * COIN;
    }
    else if (nHeight <= 2550007 && nHeight >= 1550007) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight <= 5050008 && nHeight >= 2550008) {
        nSubsidy = 1 * COIN;
    }
    else if (nHeight <= 7550009 && nHeight >= 5050009) {
        nSubsidy = 0.5 * COIN;
    }
    else if (nHeight <= 10050010 && nHeight >= 7550010) {
        nSubsidy = 0.25 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from VoucherCoin
...
{
    RenameThread("vouchercoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZVCOMinted()
{
...
void RecalculateZVCOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zVCO supply
...
bool RecalculateVCOSupply(int nHeightStart)
{
...
bool UpdateZVCOSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateVCOSupply(Params().Zerocoin_StartHeight());
    }

    //Track zVCO money supply in the block index
    if (!UpdateZVCOSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zVCO supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert VCO to zVCO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zVCO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zVCO serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zVCO stake block is less than allowed start height", __func__);

    if (CZPivStake* zVCO = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zVCO->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zVCO stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zVCO stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zVCO->GetValue()));
        if (nChecksum200 != zVCO->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zVCO->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZVCO() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zVCO fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zVCO mints and %d zVCO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // VoucherCoin: We use certain sporks during IBD, so check to see if they are
