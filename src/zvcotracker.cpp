...
CzVCOTracker::CzVCOTracker(std::string strWalletFile)
{
...
CzVCOTracker::~CzVCOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVCOTracker::Init()
{
...
bool CzVCOTracker::Archive(CMintMeta& meta)
{
...
bool CzVCOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVCOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVCOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVCOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzVCOTracker::GetSerialHashes()
{
...
CAmount CzVCOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVCOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVCOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVCOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVCOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVCOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVCOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVCOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVCOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVCOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVCOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVCOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVCOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVCOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVCOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVCOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVCOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzvco from DB\n", __func__, listDeterministicDB.size());
    }
