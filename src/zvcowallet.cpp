...
CzVCOWallet::CzVCOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvco seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVCOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVCOSeed_deprecated()) {
                    LogPrintf("%s: Updated zVCO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvco seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvco wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVCOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVCOCount(nCountLastUsed);
    else if (!walletdb.ReadZVCOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVCOWallet::Lock()
{
    seedMaster = 0;
}

void CzVCOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVCOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVCO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVCOWallet::LoadMintPoolFromDB()
{
...
void CzVCOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVCOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVCOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvcoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVCOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVCO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvcoTracker which also adds to database
    pwalletMain->zvcoTracker->Add(dMint, true);
...
void CzVCOWallet::SeedToZVCO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVCOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVCOWallet::GenerateDeterministicZVCO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVCOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVCOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
